...
class Orchestrator:
    ...
    def build_brief(
        ...
        user = (
            ...
            "DELIVERABLE_PATHS marker (REQUIRED — last line of your final assistant message):\n"
            "When you finish executing, your final assistant message MUST end with a single line in this exact form:\n\n"
            "    DELIVERABLE_PATHS: <comma-separated list of workspace-relative paths the user explicitly asked for>\n\n"
            "Rules for the marker:\n"
            "- List ONLY user-facing files. The APK / PDF / report / binary the user wants — not gradlew, not node_modules, not build/ scaffolding, not source files when a binary is the deliverable.\n"
            "- For research / Q&A tasks where the deliverable is a single document, list that one path.\n"
            "- For build tasks where the deliverable is a binary (APK / IPA / JAR / .pdf / .zip), list the binary itself, NOT the source files.\n"
            "- Use the exact relative path from the workspace root (e.g. `app/build/outputs/apk/debug/app-debug.apk`, not `./app-debug.apk` or an absolute path).\n"
            "- If the task is purely conversational and produced no files, write `DELIVERABLE_PATHS: (none)`.\n"
            "- Always include this line, even when escalating — list whatever was produced before the wall.\n\n"
